...
import os
import sys
sys.path.append("/home/helei/biosoft/mambaforge/envs/clair3/bin")
from itertools import accumulate

import clair3.model_cyclone as model_path
...
class DataSequence(tf.keras.utils.Sequence):
    def __init__(self, data, chunk_list, param, tensor_shape, cyclone, mini_epochs=1, add_indel_length=False, validation=False):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        for chunk_idx, (bin_id, chunk_id) in enumerate(chunk_batch_list):
            ...
            if self.cyclone:
                input_matrix = self.data[bin_id].root.position_matrix[start_pos:start_pos + self.chunk_size]
                pad = np.array([[0, 0], [1, 0], [0, 0]])
                output_matrix = tf.pad(input_matrix, paddings=pad, constant_values=34)
                pad = np.array([[0, 0], [0, 1], [0, 0]])
                self.position_matrix[chunk_idx * self.chunk_size:(chunk_idx + 1) * self.chunk_size] =  tf.pad(output_matrix, paddings=pad, constant_values=35)
            else:
...
def train_model(args):
    ...
    exclude_training_samples = None
    random_validation =  True
    validation_fn = None
    add_validation_dataset = random_validation or (validation_fn is not None)
    ochk_prefix = "/home/helei/work/SNP_calling/HG004/Train_test/pileup_train_test/pileup"
    bert_config = args.config
    if pileup:
        import shared.param_p as param
        if Cyclone:
            model = model_path.Clair3_bert_P(bert_config)
        else:
