...
from clair3.Bert_utils import tf_utils
...
class BertModel(tf.keras.layers.Layer):
  ...
  def __init__(self, config, float_type=tf.float32, **kwargs):
    ...
    self.config = (BertConfig.from_json_file(config))
  ...
  def call(self, inputs, mode="bert", **kwargs):
    ...
    embedding_tensor = self.embedding_postprocessor(
        word_embeddings=input_word_ids, token_type_ids=input_type_ids)
    if self.float_type == tf.float16:
      embedding_tensor = tf.cast(embedding_tensor, tf.int32)
    attention_mask = None
