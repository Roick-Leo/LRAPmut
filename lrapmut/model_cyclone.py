...
## tf bert
###########################################################################
from clair3.bert_modeling import BertConfig, BertModel
class Clair3_bert_P(tf.keras.Model):
    # Bi-lstm model for clair3 pileup input
    def __init__(self, config, add_indel_length=False, predict=False):
        super(Clair3_bert_P, self).__init__()
        self.config = config
        # output
        ...
        self.bert_layer = BertModel(self.config,float_type=tf.int32)
    ...
    def call(self, x,):
        ...
        input_mask = tf.ones_like(x)

        input_type_ids = tf.ones_like(x)

        x = self.bert_layer([x,input_mask,input_type_ids])  # (batch_size, inp_seq_len, d_model)
